//! Machine operations: the same for every front end.
...
use crate::{host_architecture, hypervisor, instance, keys, process, qmp};
use std::path::Path;
use std::time::{Duration, Instant};

/// What an operation says as it goes, for a front end to show.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Event {
    /// A fetch is starting from this URL.
    Fetching(String),
    Received(crate::store::Progress),
    Converting(u8),
    Cloning(crate::clone::Stage, u8),
    /// The guest has been asked to shut down and has this long.
    Shutdown {
        name: String,
        seconds: u64,
    },
    Exporting(crate::store::Progress),
    /// An import pass, and whether the source is a URL rather than a file.
    Importing {
        from_url: bool,
        event: crate::import::Event,
    },
}

pub type Observer<'a> = &'a mut dyn FnMut(Event);
...
pub fn run(
    ...
    let reference: crate::Reference = request.reference.parse()?;
    ...
    match (pull, held) {
        ...
        (Pull::Always, _) | (Pull::Missing, false) => {
            ...
            let _: Pulled = fetch(store, artifact, observe)?;
    ...
    if resolved.ssh_config {
        crate::ssh_config::check_name(&user_ssh_config()?, &name)?;
    }
...
/// Fetches an image, reporting its progress.
pub fn fetch(
    store: &Store,
    artifact: &crate::catalogue::Artifact,
    observe: Observer,
) -> Result<Pulled> {
    let observe = std::cell::RefCell::new(observe);
    store.pull(
        artifact,
        &crate::store::http_agent(),
        &mut |update| observe.borrow_mut()(Event::Received(update)),
        Some(&mut |percent| observe.borrow_mut()(Event::Converting(percent))),
    )
}
...
fn launch(directory: &Directory, held: &mut Instance) -> Result<Option<bool>> {
    ...
    let handle = match crate::hypervisor::Detached.start(&hypervisor::Launch {
...
fn publish(
    ...
    crate::ssh_config::write(held, directory)?;
    let user = user_ssh_config()?;
    Ok(crate::ssh_config::ensure_include(&user, instances)?.then_some(user))
}

fn user_ssh_config() -> Result<std::path::PathBuf> {
    crate::paths::ssh_config().ok_or(Error::NoHome)
}
...
fn apply(directory: &Directory, held: &mut Instance, changes: &Changes) -> Result<()> {
    ...
    if let Some(wanted) = changes.ssh_config {
        ...
        if !wanted {
            crate::ssh_config::remove(directory)?;
        }
    }
...
/// The `ssh` arguments that reach a machine, or why none can.
pub fn ssh_arguments(name: &str, command: &[String]) -> Result<Vec<String>> {
    let instances = Instances::discover()?;
    let directory = instances.open(name)?;
...
pub fn scp_arguments(from: &str, to: &str) -> Result<Vec<String>> {
    ...
    let held = directory.read()?;
    answering(&held)?;
    crate::access::scp(&held, &directory, &from, &to)
}
...
pub fn clone(
    ...
    let reference: crate::Reference = target.parse()?;
    let target = crate::clone::Target {
        reference: &reference,
        description,
    };
    let instances = Instances::discover()?;
    ...
    let local = crate::paths::local_catalogue_directory().ok_or(Error::NoImageStore)?;

    let running = held.is_running();
    let consistency = match (running, force) {
        (false, _) => reports::Consistency::Stopped,
        (true, false) => reports::Consistency::Paused,
        (true, true) => reports::Consistency::Running,
    };

    let cloned = if consistency == reports::Consistency::Paused {
        let mut client = qmp::connect(&held.monitor)?;
        // A machine the user paused stays paused.
        let was_running = client.status()? == "running";
        client.pause()?;
        // Resume whether or not the clone succeeds.
        let outcome = crate::clone::image(
            &store,
            &local,
            &held,
            &directory.overlay(),
            target,
            true,
            Some(&mut |stage, percent| observe(Event::Cloning(stage, percent))),
        );
        if was_running {
            client.resume()?;
        }
...
pub fn remove_image(
    ...
    let parsed: crate::Reference = reference.parse()?;
    ...
    let local = entry.catalogue == crate::config::STORE_CATALOGUE;
...
pub fn prune(
    ...
    let machines = if target == Some(PruneTarget::Images) {
        ...
        crate::prune::machines(&instances, store, catalogue, all)?
    };
    let images = if target == Some(PruneTarget::Machines) {
        Vec::new()
    } else {
        let used = crate::prune::in_use(&instances, store, &machines)?;
        crate::prune::images(store, catalogue, &used, all, std::time::SystemTime::now())
    };
    let items: Vec<_> = machines.into_iter().chain(images).collect();
    if !dry_run {
        crate::prune::remove(&items)?;
    }
...
/// A console log read as it grows.
pub struct Tail {
    directory: Directory,
    file: std::fs::File,
}
...
impl Tail {
    /// Opens a machine's console, or nothing if it has never been written.
    pub fn open(name: &str) -> Result<Option<Self>> {
        let instances = Instances::discover()?;
        let directory = instances.open(name)?;
        let console = directory.console();
        match std::fs::File::open(&console) {
            Ok(file) => Ok(Some(Self { directory, file })),
    ...
    /// What has been written since the last read, or everything on the first.
    ...
    /// Whether the machine is still running, so more may come.
    pub fn running(&self) -> bool {
        self.directory.read().is_ok_and(|held| held.is_running())
    }
}
...
mod tests {
    ...
    fn an_ssh_configuration_entry_is_asked_for_and_taken_away() {
        ...
        crate::ssh_config::write(&held, &directory).unwrap();
